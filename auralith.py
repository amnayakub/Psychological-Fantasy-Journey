...
# 🧠 Initialize Psychological Traits with Sub-traits
stats = {
    'Openness': {
        'score': 50,
...
def scene_bridge():
    ...
    if choice == '1':
        ...
        stats['Neuroticism']['score'] += 3  # Reduced from 4
        stats['Openness']['score'] += 3
        print_slowly(
...
def scene_masquerade():
    ...
    if choice == '1':
        ...
        # Reduced slightly for better accuracy
        stats['Neuroticism']['score'] += 3
        stats['Agreeableness']['score'] += 2
        print_slowly(
...
# Additional scenes would follow similarly...
...
for trait in stats:
    stats[trait]['score'] = max(
        0, min(100, stats[trait]['score']))  # Clamp between 0-100
...
# Optional: Display as DataFrame Table
